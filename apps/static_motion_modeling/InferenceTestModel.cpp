    ...
    for (const auto& image_path : image_paths) {
        ...
        for (int r = 0; r < img.rows; ++r) {
            for (int c = 0; c < img.cols; ++c) {
                tensor.at(static_cast<size_t>(r), static_cast<size_t>(c)) = static_cast<float>(img.at<uchar>(r, c)) / 255.0f;
            }
        }
        ...
        onoff.process_test_sample(label, tensor, 0, image_paths.size());
        std::cout << "Tensor shape: " << tensor.shape().to_string() << std::endl;

        scale.process_test_sample(label, tensor, 0, image_paths.size());
        // save_tensor_as_image(tensor, "debug_outputs/scaled.png");
        std::cout << "Tensor shape: " << tensor.shape().to_string() << std::endl;



        code.process_test_sample(label, tensor, 0, image_paths.size());
        // save_tensor_as_image(tensor, "debug_outputs/latency_encoded.png");


        conv.process_test_sample(label, tensor, 0, image_paths.size());
        // save_tensor_as_image(tensor, "debug_outputs/after_conv.png");


        sum_pool.process_test_sample(label, tensor, 0, image_paths.size());
        scaling.process_test_sample(label, tensor, 0, image_paths.size());
        // save_tensor_as_image(tensor, "debug_outputs/final_features.png");
